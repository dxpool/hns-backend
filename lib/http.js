  ...
  initRouter() {
    ...
    this.get('/summary', async (req, res) => {
      ...
      res.json(200, {
        ...
        registeredNames: this.hnscan.nameList.length
      });
    });
    ...
    this.get('/addresses/:hash', async (req, res) => {
      ...
      const data = this.cache.get(hash + ':balance');
      if (data) {
        res.json(200, data);
        return;
      }
      ...
      this.cache.set(hash + ':balance', balance);
      ...
      try {
        switch (type) {
          ...
          case 'dailyTotalTransactions':
            chartData = await this.hdb.getTotalTransactionSeries(startTime, endTime);
            break;
          case 'supply':
    ...
    this.get('/pool/distribution', async (req, res) => {
      ...
      const data = await this.hnscan.getPoolCount(startTime, endTime);
