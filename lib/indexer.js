...
class Indexer extends EventEmitter {
  ...
  async close() {
    ...
    this.client.removeAllListeners('block connect');
    this.client.removeAllListeners('chain reset');
  ...
  async indexTX(entry, block) {
    ...
    for (const tx of block.txs) {
      ...
      if (coins.length >= BATCH_SIZE) {
        await this.db.coinDb.bulkWrite(coins.splice(0, BATCH_SIZE));
      }
      if (names.size >= BATCH_SIZE) {
        const nameArray = Array.from(names.entries()).slice(0, BATCH_SIZE);
        await this.db.nameDb.bulkWrite(nameArray.map(([key, value]) => value));
        nameArray.forEach(([key]) => names.delete(key));
      }
      if (txs.length >= BATCH_SIZE) {
        await this.db.txDb.bulkWrite(txs.splice(0, BATCH_SIZE));
      }
    }
