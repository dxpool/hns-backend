...
class Hnscan extends EventEmitter {
  ...
  async getTransactionsByAddress(addr, limit = 25, offset = 0) {
    ...
    const items = this.db.txDb.find({ addresses: address })
      ...
      .skip(actualOffset)
      .limit(actualLimit)
      .project({ txid: 1, height: 1, time: 1 }); // 只查询必要字段，减少内存占用

    const result = [];
    const txIds = [];

    try {
      // 先收集所有txid，避免在循环中频繁查询
      while (await items.hasNext()) {
        const item = await items.next();
        txIds.push(Buffer.from(item.txid, 'hex'));

        // 限制批量大小，避免一次性加载太多交易
        if (txIds.length >= actualLimit) break;
      }
    } finally {
      // 确保游标被关闭
    ...
    // 批量获取交易，但限制并发数
    const CONCURRENT_LIMIT = 5; // 最多同时查询5个交易
    for (let i = 0; i < txIds.length; i += CONCURRENT_LIMIT) {
      const batch = txIds.slice(i, i + CONCURRENT_LIMIT);
      const batchResults = await Promise.all(
        batch.map(txid => this.getTransaction(txid).catch(err => {
          this.logger.error('Error getting transaction: %s', err.message);
          return null;
        }))
      );
