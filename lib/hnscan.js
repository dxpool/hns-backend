...
class Hnscan extends EventEmitter {
  ...
  async getNamesByStatus(status, limit = 10, offset = 0) {
    ...
    const count = await this.db.nameDb.countDocuments(options);
  ...
  async getHighBidNames(type, limit = 10, offset = 0) {
    let span = 7 * 24 * 60 * 60;
    switch (type) {
      case 'monthBid':
        span = 30 * 24 * 60 * 60;
    ...
    while (await items.hasNext()) {
      const item = await items.next();
      const ns = await this.chain.db.getNameState(Buffer.from(item._id, 'hex'));
      const info = await ns.getJSON(this.chain.height, this.network);
  ...
  async getNameBids(nameHash, open) {
    const items = this.db.coinDb.aggregate()
      ...
      .lookup({
        from: 'coins',
        let: { spentTxid: '$spentTxid', spentIndex: '$spentIndex' },
        pipeline: [{
          $match: {
            $expr: {
              $and: [{
  ...
  async getNameCount() {
    return this.db.nameDb.countDocuments();
  }

  async getSeries(type, start, end) {
    const items = this.db.summaryDb.aggregate()
      .match({
    ...
    switch (type) {
      ...
      case 'supply':
        while (await items.hasNext()) {
          ...
          result.push({
            date: item.time * 1000,
            value: item.supply / Math.pow(10, 6)
          });
        }
        break;
      ...
      case 'burned':
        while (await items.hasNext()) {
          ...
          result.push({
            date: item.time * 1000,
            value: item.burned / Math.pow(10, 6)
          });
        }
        break;
    }
