...
class Hnscan extends EventEmitter {
  ...
  async getRecentHighestNames() {
    try {
      ...
      for (let i = this.chain.height; i >= -1; i--) {
        let block = this.blockNames.get(i);
        ...
        if (!block) {
          ...
          for (const tx of data.txs) {
            ...
            for (const { name, value, action } of json.outputs) {
              if (action === 'BID') {
          ...
          block = { time: data.time, info: info };
          this.blockNames.set(i, block);
        }
      ...
      for (let i = end; i >= 0; i--) {
        if (this.blockNames.get(i)) {
          this.blockNames.delete(i);
        } else {
          break;
        }
      }
